...
#物体だけを取り出す
def Cut_draw(img_path, bbox_Coordinate, img_number, FM_name):
    #画像読み込み
    ...
    for item_position in bbox_Coordinate:
        ...
        #画像出力
        img_crop.save("./data/images_val/" + FM_name + "/cut_image_drink{0}.jpg".format(img_number))
        #座標リストを空にする
        item_position_list.clear()

if __name__ == '__main__':
    FM_name = ["./data/validation/men","./data/validation/women"]
    for FM in FM_name:
        #使用する画像のパスを指定する
        img_path = FM + "/*.jpg"
        #１つずつ画像パスを読み取る
        ...
        os.makedirs("./data/images_val/" + FM, exist_ok=True)
        #画像を１つずつ処理する
        for img in img_jpg:
            ...
            if type(bbox_Coordinate) is not int:
                #画像の抜き取り
                Cut_draw(img, bbox_Coordinate, img_number, FM)
                img_number += 1