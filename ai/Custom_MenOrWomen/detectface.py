...
def Cut_draw(img_path, bbox_Coordinate, img_number, FM_name):
    ...
    for item_position in bbox_Coordinate:
        ...
        #画像出力
        img_crop.save("./add_testdata/picked2/" + FM + "/{0}.jpg".format(img_number))
        #座標リストを空にする
        item_position_list.clear()

if __name__ == '__main__':
    FM_name = ["./add_testdata/men2","./add_testdata/women"]
    for FM in FM_name:
        #使用する画像のパスを指定する
        img_path = FM + "/*.jpg"
        #１つずつ画像パスを読み取る
        ...
        os.makedirs("./add_testdata/picked2/"+FM, exist_ok=True)
        #画像を１つずつ処理する
        for img in img_jpg:
