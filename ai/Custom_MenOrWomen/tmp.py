...
FM_list = ["manre","womanre"]
for FM_name in FM_list:

    #フォルダーを作成している
    os.makedirs("./newdata/tmp/"+ FM_name, exist_ok=True)

    #読み取る画像のフォルダを指定
    in_dir = "./newdata/"+FM_name+"/*.jpg"
    #出力先のフォルダを指定
    out_dir = "./newdata/tmp/"+FM_name
    ...
    for i in range(len(img_jpg)):
        ...
        #パスを結合している
        fileName = os.path.join(out_dir+"/"+"_"+str(i)+".jpg")
        #画像を出力
